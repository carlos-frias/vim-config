...
let g:loaded_netrwPlugin = "v153p"
if v:version < 702
 echohl WarningMsg
...
"   call Decho("getcwd<".getcwd().">")
  ...
  elseif !a:islocal && dirname !~ '[\/]$' && dirname !~ '^"'
   " s:NetrwBrowse: looks like a remote regular file {{{3
...
doc/pi_netrw.txt	[[[1
3643
*pi_netrw.txt*  For Vim version 7.4.  Last change: 2014 Jul 30
...
<	 <F1>	Causes Netrw to issue help
	 ...
	 <c-r>	Browsing using a gvim server                      |netrw-ctrl-r|
...
best seen with a special handler (ie. a tool provided with your computer o/s).
...
One may also use visual mode (see |visual-start|) to select the text that
the special handler will use.  Normally gx uses expand("<cfile>") to pick
up the text under the cursor; one may change what |expand()| uses via the
|g:netrw_gx| variable.
...
:Hexplore! [dir] does an :Explore with |:aboveleft|  horizontal splitting.
						*netrw-:Lexplore*
:[count]Lexplore [dir] toggles an Explorer window on the left hand side
	  ...
	  change an uninitialized |g:netrw_chgwin| to 2; edits will thus be
	  preferentially made in window#2.  The [count] specifies a
	  |g:netrw_winsize| just for the new :Lexplore window.
	  Also see: |netrw-C|| |g:netrw_chgwin| |g:netrw_winsize|
		    |netrw-p| |netrw-P|| |g:netrw_browse_split|
:[count]Lexplore! is like :Lexplore, except that the Explorer window will open
	  on the right hand side, and an uninitialized |g:netrw_chgwin|
	  will be set to 1.
						*netrw-:Sexplore*
